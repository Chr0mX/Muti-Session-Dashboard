...
- `scripts/Dashboard.ps1` is the WinForms UI **only** — Create User, Start, Connect RDP, Stop, Refresh, and Open RDP Wrapper buttons, a grid, and a 2-second status timer. It holds no session-management logic itself; every button click and every timer tick just dispatches a named module command and renders whatever comes back.
  ...
  - **RDP Tweaks** runs the installed copy of `BetterRDP.ps1` (see below) via `Invoke-DashboardBetterRdpTweak` in `scripts/RdpManager.psm1`. Choosing **Yes** applies the default optimizations (machine-wide registry changes; a reboot is required afterward); **No** runs validation only, which is read-only and reports the current match/mismatch status against the expected values. Either way the script's own output is shown in a dialog. Runs as a child `powershell.exe` process rather than in-runspace, since `BetterRDP.ps1` declares a `class` at its top level that can't be redefined in an already-warm runspace.
  - A background monitor polls every Remote Desktop Users member's real session state every 2 seconds, so the dashboard reflects RDP connections made outside the dashboard too (e.g. a manual `rdp.exe`/mstsc connection) — a pure status reflection of live Windows state, never a stored flag alone. **Every RDS user is kept headless-armed automatically**: whenever a user's RDP status shows Disconnected (whether their interactive session just closed, they were never started at all, or a previous arm attempt failed) and they haven't been explicitly Stopped, the monitor waits for their session to genuinely finish closing and re-arms the headless loopback right there — no manual Start required, ever, for a user that hasn't been Stopped. If an automatic re-arm attempt fails, the grid's **Last Error** column shows the real exception message directly (cleared again on the next successful arm/connect) instead of the failure being silently swallowed.
...
- `scripts/BetterRDP/` contains [Upinel/BetterRDP](https://github.com/Upinel/BetterRDP)'s host-side RDP performance tweaks (`UpinelBetterRDP.reg`, `BetterRDP.ps1`). `BetterRDP.ps1` is copied to the install root by both `install.ps1` and `scripts/Install-MultiSessionDashboard.ps1`, and is run from the dashboard's **RDP Tweaks** button above (backup/apply/apply-gaming/validate, non-interactively via its `-Action` parameter) as well as interactively (its original `Read-Host` 1-6 menu, unchanged, still works when run directly with no `-Action`). `UpinelBetterRDP.reg` is copied alongside it but stays a manual double-click import -- there's no dashboard button for it. The `.ps1` copy here is modified to detect the RDP server's current primary monitor refresh rate and match the frame-rate-related registry values to it, instead of the upstream script's fixed ~60Hz assumption.
...
A headless loopback connection is just an ordinary automated RDP login (same as Connect RDP) that gets minimized right after it comes up, so it never shows a window, and titled `RDP-<user>-Headless` via Remote Desktop Plus's own `/title:` flag (a real interactive session is titled `RDP-<user>` the same way) so the two are distinguishable in Task Manager if you ever need to check. Its only purpose is to keep the account's session alive and immediately reconnect-able. Nothing about it is special at the RDP protocol level — the "displacement" when Connect RDP is used afterward is just RDP's normal single-active-client-per-session behavior.
